...
UW_INDEX_CONTEXT_SYMBOLS = {SYMBOL, "SPX", "SPXW", "QQQ", "IWM"}
...
def summarize_unusual_whales_option_contracts(payload: dict | None, expiration_date=None, latest_price: float | None = None) -> dict | None:
    expiry_target = str(expiration_date) if expiration_date is not None else ""
    rows = []
    for row in payload_rows(payload):
        row_expiry = str(row.get("expiry") or row.get("expiration") or row.get("expiration_date") or "")
        ...
        if latest_price is not None and not pd.isna(latest_price) and abs(strike - float(latest_price)) > 12:
            continue
        bid = _finite_float(row.get("bid") or row.get("bid_price"))
        ask = _finite_float(row.get("ask") or row.get("ask_price"))
        mark = _finite_float(row.get("mark") or row.get("mid") or row.get("last") or row.get("price"))
        volume = _finite_float(row.get("volume") or row.get("vol"), 0.0)
        open_interest = _finite_float(row.get("open_interest") or row.get("openInterest") or row.get("oi"), 0.0)
        premium = _finite_float(row.get("premium") or row.get("total_premium"), 0.0)
        liquidity_score = volume * 1.5 + open_interest + premium / 1000.0
        rows.append({
            "type": option_type,
            "strike": strike,
            "volume": volume,
            ...
            "delta": None if pd.isna(_finite_float(row.get("delta"))) else _finite_float(row.get("delta")),
            "iv": None if pd.isna(_finite_float(row.get("iv") or row.get("implied_volatility"))) else _finite_float(row.get("iv") or row.get("implied_volatility")),
            "symbol": row.get("option_symbol") or row.get("symbol") or row.get("contract"),
            "liquidity_score": liquidity_score,
        })
    if not rows:
        return None
    calls = [row for row in rows if row["type"] == "CALL"]
    puts = [row for row in rows if row["type"] == "PUT"]

    def _rank(side_rows: list[dict]) -> list[dict]:
        return sorted(side_rows, key=lambda row: (row.get("liquidity_score", 0.0), row.get("volume", 0.0), row.get("open_interest", 0.0)), reverse=True)[:8]

    by_strike: dict[float, dict] = {}
    for row in rows:
        bucket = by_strike.setdefault(row["strike"], {"strike": row["strike"], "call_volume": 0.0, "put_volume": 0.0, "call_open_interest": 0.0, "put_open_interest": 0.0, "liquidity_score": 0.0})
        prefix = "call" if row["type"] == "CALL" else "put"
        bucket[f"{prefix}_volume"] += _finite_float(row.get("volume"), 0.0)
        bucket[f"{prefix}_open_interest"] += _finite_float(row.get("open_interest"), 0.0)
        bucket["liquidity_score"] += _finite_float(row.get("liquidity_score"), 0.0)
    strike_rows = sorted(by_strike.values(), key=lambda row: row.get("liquidity_score", 0.0), reverse=True)[:8]
    return {
        "contract_count": len(rows),
        "top_calls": _rank(calls),
        "top_puts": _rank(puts),
        "liquid_strikes": strike_rows,
        "call_volume": sum(_finite_float(row.get("volume"), 0.0) for row in calls),
        "put_volume": sum(_finite_float(row.get("volume"), 0.0) for row in puts),
        "call_open_interest": sum(_finite_float(row.get("open_interest"), 0.0) for row in calls),
        "put_open_interest": sum(_finite_float(row.get("open_interest"), 0.0) for row in puts),
        "as_of": str(pd.Timestamp.now(tz=get_central_tz())),
    }
...
def merge_payload_rows(*payloads: dict | list | None) -> dict:
    rows: list[dict] = []
    seen: set[str] = set()
    for payload in payloads:
        for row in payload_rows(payload):
            key = str(row.get("id") or row.get("trade_id") or row.get("transaction_id") or "")
            if not key:
                key = "|".join(str(row.get(part) or "") for part in ("ticker", "symbol", "executed_at", "timestamp", "price", "size", "premium"))
            if key in seen:
                continue
            seen.add(key)
            rows.append(row)
...
def _uw_screener_symbol(row: dict) -> str:
    symbol = str(row.get("ticker_symbol") or row.get("ticker") or row.get("underlying_symbol") or row.get("symbol") or "").upper()
    if symbol:
        return symbol
    contract = str(row.get("option_symbol") or row.get("contract") or "").upper()
    match = re.match(r"([A-Z]+)", contract)
    return match.group(1) if match else ""


def summarize_unusual_whales_whale_targets(call_payload: dict | None, put_payload: dict | None, now_ct, latest_price: float | None = None) -> dict | None:
    rows = []
    for payload, fallback_type in [(call_payload, "CALL"), (put_payload, "PUT")]:
        for row in payload_rows(payload):
            symbol = _uw_screener_symbol(row)
            if symbol not in UW_INDEX_CONTEXT_SYMBOLS:
                continue
            timestamp_keys = ("created_at", "updated_at", "timestamp", "time", "date")
            has_timestamp = any(row.get(key) for key in timestamp_keys)
            if has_timestamp and not row_is_current_for_0dte(row, now_ct, timestamp_keys):
                continue
            option_type = _option_row_type(row)
            if option_type not in {"CALL", "PUT"}:
                option_type = fallback_type
            strike = _strike_key(row.get("strike"))
            if strike is None:
                continue
            premium = _row_trade_premium(row)
            volume = _finite_float(row.get("volume") or row.get("vol") or row.get("total_volume"), 0.0)
            open_interest = _finite_float(row.get("open_interest") or row.get("openInterest") or row.get("oi"), 0.0)
            vol_oi = _finite_float(row.get("volume_oi_ratio") or row.get("vol_oi_ratio") or row.get("volume_to_oi"))
            distance = abs(strike - float(latest_price)) if latest_price is not None and not pd.isna(latest_price) else float("nan")
            rows.append({
                "symbol": symbol,
                "type": option_type,
                "strike": strike,
                ...
                "volume": volume,
                "open_interest": open_interest,
                "volume_oi_ratio": None if pd.isna(vol_oi) else vol_oi,
                "distance": None if pd.isna(distance) else distance,
                "expiration": row.get("expiry") or row.get("expiration") or row.get("expiration_date"),
            })
    if not rows:
        return None
    spy_rows = [row for row in rows if row["symbol"] == SYMBOL]
    target_rows = spy_rows or rows
    call_premium = sum(_finite_float(row.get("premium"), 0.0) for row in target_rows if row.get("type") == "CALL")
    put_premium = sum(_finite_float(row.get("premium"), 0.0) for row in target_rows if row.get("type") == "PUT")
    if call_premium > put_premium * 1.2 and call_premium > 0:
        tone = "Whale targets lean calls"
        side = "CALL"
    elif put_premium > call_premium * 1.2 and put_premium > 0:
        tone = "Whale targets lean puts"
        side = "PUT"
    else:
        tone = "Whale targets are balanced"
        side = "MIXED"
    top_targets = sorted(target_rows, key=lambda row: (_finite_float(row.get("premium"), 0.0), _finite_float(row.get("volume"), 0.0)), reverse=True)[:8]
    return {
        "tone": tone,
        "side": side,
        "target_count": len(target_rows),
        "market_context_count": len(rows),
        "call_premium": call_premium,
        "put_premium": put_premium,
        "top_targets": top_targets,
        "as_of": str(pd.Timestamp(now_ct)),
    }


def uw_news_row_is_spy_relevant(row: dict) -> bool:
    tickers = row.get("tickers") or row.get("symbols") or row.get("ticker") or row.get("symbol")
    if isinstance(tickers, list):
        if any(str(item).upper() in {SYMBOL, "SPX", "ES", "QQQ", "VIX", "^VIX"} for item in tickers):
            return True
    elif tickers and str(tickers).upper() in {SYMBOL, "SPX", "ES", "QQQ", "VIX", "^VIX"}:
        return True
    text = f"{row.get('title') or row.get('headline') or ''} {row.get('summary') or ''}".lower()
    keywords = ("spy", "s&p", "spx", "futures", "fed", "inflation", "jobs", "yields", "treasury", "vix", "market", "nasdaq")
...
def summarize_unusual_whales_gex(payload: dict | None, latest_price: float | None = None) -> dict:
    ...
    for row in payload_rows(payload):
        ...
        call_dex = _finite_float(
            row.get("call_dex"),
            _finite_float(row.get("call_delta_oi"), _finite_float(row.get("call_delta_exposure"), 0.0)),
        )
        put_dex = _finite_float(
            row.get("put_dex"),
            _finite_float(row.get("put_delta_oi"), _finite_float(row.get("put_delta_exposure"), 0.0)),
        )
    ...
    net_dex = sum(row.get("total_dex", 0.0) for row in rows)
    tone = "Stabilizing positive gamma" if net > 0 else "Volatile negative gamma" if net < 0 else "Neutral gamma"
    dex_tone = "Call-side dealer delta" if net_dex > 0 else "Put-side dealer delta" if net_dex < 0 else "Balanced dealer delta"
    dex_ranked = sorted(rows, key=lambda row: abs(row.get("total_dex", 0.0)), reverse=True)[:6]
    return {
        "gamma_flip": gamma_flip,
        "dealer_tone": tone,
        "levels": ranked,
        "dex_levels": dex_ranked,
        "net_gex": net,
        "net_dex": net_dex,
        "dex_tone": dex_tone,
    }
...
def merge_unusual_whales_gex(spot_payload: dict | None, static_payload: dict | None, latest_price: float | None = None) -> dict:
    spot = summarize_unusual_whales_gex(spot_payload, latest_price)
    static = summarize_unusual_whales_gex(static_payload, latest_price)
    if spot.get("levels"):
        return {
            **spot,
            "source": "spot",
            "static_levels": static.get("levels") or [],
            "static_gamma_flip": static.get("gamma_flip"),
        }
    if static.get("levels"):
        return {**static, "source": "static", "static_levels": static.get("levels") or [], "static_gamma_flip": static.get("gamma_flip")}
    return {**spot, "source": "pending", "static_levels": static.get("levels") or [], "static_gamma_flip": static.get("gamma_flip")}


def premium_feed_coverage(whales: dict | None) -> dict:
    if not isinstance(whales, dict):
        return {"loaded": 0, "total": 0, "label": "Premium feeds pending", "feeds": {}}
    checks = {
        "flow_alerts": bool((whales.get("flow_alerts") or {}).get("alert_count")),
        "recent_flow": bool(whales.get("recent_flow")),
        "market_tide": bool(whales.get("market_tide")),
        "net_premium": bool(whales.get("net_premium_ticks")),
        "options_volume": bool(whales.get("options_volume")),
        "contract_liquidity": bool(whales.get("contract_liquidity")),
        "iv": bool(whales.get("interpolated_iv")),
        "gex": bool((whales.get("gex") or {}).get("levels")),
        "greeks": bool(whales.get("greeks")),
        "darkpool": bool(whales.get("darkpool")),
        "whale_targets": bool(whales.get("whale_targets")),
        "news": bool(whales.get("fresh_news")),
    }
    loaded = sum(1 for value in checks.values() if value)
    total = len(checks)
    return {"loaded": loaded, "total": total, "label": f"Premium feeds {loaded}/{total}", "feeds": checks}
...
def fetch_unusual_whales_intelligence(expiration_date, latest_price: float | None = None, now_ct=None) -> tuple[dict | None, SourceStatus]:
    ...
    if latest_price is None or pd.isna(_finite_float(latest_price)):
        return None, unusual_whales_status(False, "Premium order-flow scan waits for a valid SPY price.", now, skipped=True)
    today = str(now.date())
    errors = []
    ...
    if err:
        errors.append(err)
    tide_payload, err = fetch_unusual_whales_json("/api/market/market-tide", (("interval_5m", "true"),))
    if err:
        errors.append(err)
    net_premium_payload, err = fetch_unusual_whales_json(f"/api/stock/{SYMBOL}/net-prem-ticks")
    if err:
        errors.append(err)
    options_volume_payload, err = fetch_unusual_whales_json(f"/api/stock/{SYMBOL}/options-volume")
    if err:
        errors.append(err)
    iv_payload, err = fetch_unusual_whales_json(f"/api/stock/{SYMBOL}/interpolated-iv")
    if err:
        errors.append(err)
    contract_payload, err = fetch_unusual_whales_json(f"/api/stock/{SYMBOL}/option-contracts")
    if err:
        errors.append(err)
    gex_payload, err = fetch_unusual_whales_json(f"/api/stock/{SYMBOL}/spot-exposures/strike")
    if err:
        errors.append(err)
    static_gex_payload, err = fetch_unusual_whales_json(f"/api/stock/{SYMBOL}/greek-exposure/strike")
    if err:
        errors.append(err)
    greeks_payload, err = fetch_unusual_whales_json(f"/api/stock/{SYMBOL}/greeks")
    if err:
        errors.append(err)
    darkpool_payload, err = fetch_unusual_whales_json(f"/api/darkpool/{SYMBOL}")
    if err:
        errors.append(err)
    market_darkpool_payload, err = fetch_unusual_whales_json("/api/darkpool/recent")
    if err:
        errors.append(err)
    whale_call_payload, err = fetch_unusual_whales_json(
    ...
    if err:
        errors.append(err)
    whale_put_payload, err = fetch_unusual_whales_json(
        "/api/screener/option-contracts",
        (
            ("limit", 100),
            ("min_premium", 250000),
    ...
    if err:
        errors.append(err)
    news_payload, err = fetch_unusual_whales_json("/api/news/headlines")
    if err:
        errors.append(err)
    ...
    contract_liquidity = summarize_unusual_whales_option_contracts(contract_payload, expiration_date, latest_price)
    darkpool = summarize_unusual_whales_darkpool(merge_payload_rows(darkpool_payload, market_darkpool_payload), now, latest_price)
    gex = merge_unusual_whales_gex(gex_payload, static_gex_payload, latest_price)
    greeks = summarize_unusual_whales_greeks(greeks_payload, expiration_date, latest_price)
    whale_targets = summarize_unusual_whales_whale_targets(whale_call_payload, whale_put_payload, now, latest_price)
    uw_news_rows = [
        row for row in payload_rows(news_payload)
        if uw_news_row_is_spy_relevant(row) and row_is_current_for_0dte(row, now, ("created_at", "published_at", "timestamp", "time", "date"))
    ][:5]
    has_data = bool(flow["alert_count"] or recent_flow or tide or net_premium or volume or iv or contract_liquidity or darkpool or whale_targets or uw_news_rows or gex.get("levels") or greeks)
    if not has_data:
    ...
    if contract_liquidity:
        bits.append("contract liquidity")
    if tide:
    ...
    if whale_targets:
        bits.append("market whale targets")
    coverage = premium_feed_coverage(payload)
    payload["coverage"] = coverage
    if coverage["total"]:
        bits.append(coverage["label"])
...
def build_options_intelligence(expiration_date, underlying_price: float | None = None, option_state: OptionsCockpitState | None = None) -> OptionsIntelligence:
    ...
    if whales_payload:
        detail += " Premium flow, contract liquidity, dark-pool, and gamma context is active."
...
def build_gamma_exposure_insight(options_intel: OptionsIntelligence) -> GammaExposureInsight:
    ...
    if isinstance(whales_gex, dict) and (whales_gex.get("levels") or whales_gex.get("gamma_flip")):
        ...
        source_label = "spot GEX" if str(whales_gex.get("source") or "spot") == "spot" else "static GEX"
        notes = f"SPY {source_label} by strike is available; use these levels as volatility/magnet context, not as standalone entries."
        return GammaExposureInsight(
            source_status("Dealer GEX", True, f"SPY {source_label} by strike available.", pd.Timestamp.now(tz=get_central_tz())),
...
def build_morning_briefing_prompt(bundle: MorningBriefingBundle) -> str:
    ...
    return (
        ...
        "If premium order-flow data is present, treat it as the primary options-flow source and explicitly weigh same-day SPY OTM flow alerts, recent tape, net premium ticks, market tide, contract liquidity, key strikes, near-strike Greeks, dark-pool levels, IV, and dealer GEX; pair that with local max pain before choosing CALL/PUT/WAIT. "
...
def fallback_morning_decision(bundle: MorningBriefingBundle, result: MorningBriefingResult | None = None) -> dict:
    ...
    decision = {
        ...
        "source_notes": ["Internal summary from the verified SPY Foresight data bundle."],
...
def rule_based_morning_briefing(bundle: MorningBriefingBundle, ai_warning: str | None = None) -> MorningBriefingResult:
    ...
    recommendation = "Wait for a clean hourly rejection at one of the structure triggers before choosing direction."
...
def external_level_rows(options: OptionsIntelligence | None) -> list[dict]:
    ...
    if isinstance(gex, dict):
        for row in gex.get("levels") or []:
            ...
            dex = _finite_float(row.get("total_dex"))
            signed = "positive" if total > 0 else "negative" if total < 0 else "neutral"
            level = _external_level_row(
                "Dealer GEX",
                row.get("strike"),
                f"{signed} GEX {fmt_money_short(total)}" + (f"; DEX {fmt_money_short(dex)}" if not pd.isna(dex) and dex else ""),
                2.25,
                magnitude=abs(_finite_float(total, 0.0)),
            )
            if level:
                rows.append(level)
        for row in gex.get("dex_levels") or []:
            if not isinstance(row, dict):
                continue
            dex = _finite_float(row.get("total_dex"))
            if pd.isna(dex) or dex == 0:
                continue
            side = "CALL" if dex > 0 else "PUT"
            level = _external_level_row(
                "Dealer DEX",
                row.get("strike"),
                f"{display_state_label(side)} delta exposure {fmt_money_short(dex)}",
                1.35,
                side,
                abs(dex),
            )
    ...
    for source, key_rows in [("Option Flow", flow.get("key_strikes") if isinstance(flow, dict) else []), ("Recent Flow", recent.get("top_strikes") if isinstance(recent, dict) else [])]:
        for row in key_rows or []:
            ...
            if level:
                rows.append(level)
    if isinstance(contract_liquidity, dict):
        for row in contract_liquidity.get("liquid_strikes") or []:
            if not isinstance(row, dict):
                continue
            call_liquidity = _finite_float(row.get("call_volume"), 0.0) + _finite_float(row.get("call_open_interest"), 0.0)
            put_liquidity = _finite_float(row.get("put_volume"), 0.0) + _finite_float(row.get("put_open_interest"), 0.0)
            side = "CALL" if call_liquidity > put_liquidity else "PUT" if put_liquidity > call_liquidity else None
            magnitude = max(call_liquidity, put_liquidity, _finite_float(row.get("liquidity_score"), 0.0))
            level = _external_level_row(
                "Contract Liquidity",
                row.get("strike"),
                f"{display_state_label(side) if side else 'balanced'} liquidity {fmt_float(magnitude, 0)}",
                0.85,
                side,
                magnitude,
            )
            if level:
                rows.append(level)
    if isinstance(whale_targets, dict):
        for row in whale_targets.get("top_targets") or []:
            if not isinstance(row, dict):
                continue
            ...
            level = _external_level_row(
                "Whale Target",
                row.get("strike"),
                f"{display_state_label(side)} target {fmt_money_short(row.get('premium'))}",
                1.45,
                side,
                row.get("premium"),
            )
...
def _level_threshold(source: str) -> float:
    if source in {"Option Flow", "Recent Flow", "Contract Liquidity", "Call Wall", "Put Wall", "Whale Target", "Dealer DEX"}:
        return 2.5
    if source == "Max Pain":
        return 1.75
    return 1.25
...
def structure_external_scenarios(bundle: MorningBriefingBundle) -> list[dict]:
    ...
    for line in structure_lines_with_values(bundle):
        ...
        for hit in hits:
            ...
            if source in {"Option Flow", "Recent Flow", "Contract Liquidity", "Call Wall", "Put Wall", "Whale Target", "Dealer DEX"} and level_side in {"CALL", "PUT"} and side in {"CALL", "PUT"}:
                if level_side == side:
...
def gamma_entry_alignment(options_intel: OptionsIntelligence | None, watch_side: str | None = None) -> dict:
    whales = premium_flow_payload(options_intel)
    gex = whales.get("gex") or {}
    iv = whales.get("interpolated_iv") or whales.get("iv") or {}
    ...
    net_dex = _finite_float(gex.get("net_dex") if isinstance(gex, dict) else None)
    ...
    if not pd.isna(net_dex) and net_dex:
        dex_side = "CALL" if net_dex > 0 else "PUT"
        if watch_side in {"CALL", "PUT"}:
            state = "aligned" if dex_side == watch_side and state != "opposes" else "opposes" if dex_side != watch_side and state == "neutral" else state
        notes.append(f"dealer DEX leans {display_state_label(dex_side).lower()}")
...
def premium_flow_tags(options_intel: OptionsIntelligence | None) -> list[str]:
    ...
    targets = str(read.get("whale_target_tone") or "").upper().replace(" ", "_")
    if targets:
        tags.append(targets[:40])
    return tags
...
def inject_global_css() -> None:
    st.markdown("""
    <style>
    @import url('https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800&family=JetBrains+Mono:wght@400;500;600;700&family=Manrope:wght@400;500;600;700;800&family=Roboto+Mono:wght@400;500;600;700&family=Space+Grotesk:wght@500;600;700&display=swap');
    :root {
      --bg:#0B1220;--surface:#131C2E;--surface2:#1A2540;--surface3:#1E2A44;
      --border:rgba(255,255,255,0.06);--border2:rgba(255,255,255,0.12);--text:#E8ECF4;--muted:#A0AEC8;
      --blue:#94A3B8;--green:#22C55E;--red:#EF4444;--amber:#F5B642;
      --cyan:#94A3B8;--shadow:none;
      --ui-font:"Inter","Manrope","Aptos","Segoe UI",system-ui,sans-serif;
      --display-font:"Inter","Space Grotesk","Manrope","Aptos","Segoe UI",system-ui,sans-serif;
      --mono-font:"JetBrains Mono","Roboto Mono","Cascadia Mono","Consolas",monospace;
    }
    ...
    div[data-baseweb="tab-list"]{display:flex;width:100%;gap:8px;border:1px solid var(--border);border-radius:8px;background:var(--surface);padding:7px;margin:2px 0 18px;box-shadow:none;overflow-x:auto;scrollbar-width:thin}
    ...
    div[data-baseweb="tab-list"] button[role="tab"]:hover{background:rgba(255,255,255,0.10);border-color:rgba(255,255,255,0.10);color:var(--text)}
    div[data-baseweb="tab-list"] button[role="tab"][aria-selected="true"]{border-color:rgba(34,197,94,.62);background:var(--surface);color:var(--text);box-shadow:none}
    .terminal-hero{border:1px solid var(--border2);border-radius:8px;background:var(--surface);box-shadow:none;padding:18px 20px;margin-bottom:14px}
    ...
    .brand-logo{position:relative;display:inline-flex;align-items:center;justify-content:center;flex:0 0 auto;width:82px;height:82px;border-radius:8px;border:1px solid rgba(255,255,255,0.10);overflow:hidden;isolation:isolate;background:var(--surface2);box-shadow:none}
    .brand-logo::before{content:"";position:absolute;inset:0;background:var(--surface);z-index:-1}
    .brand-logo svg{width:82%;height:82%;overflow:visible}.brand-grid{stroke:rgba(215,240,255,.16);stroke-width:.8}.brand-mono{fill:var(--text);font:900 9px var(--mono-font);letter-spacing:.4px}.brand-path{fill:none;stroke:var(--amber);stroke-width:2.2;stroke-linecap:round;stroke-linejoin:round;stroke-dasharray:34;animation:brandDraw 2.9s ease-in-out infinite}.brand-dot{fill:var(--green);animation:brandPulse 1.45s ease-in-out infinite}.brand-orbit{fill:none;stroke:rgba(246,251,255,.48);stroke-width:1.4;stroke-dasharray:4 4;animation:brandOrbit 7s linear infinite;transform-origin:24px 24px}.brand-scan{stroke:#94A3B8;stroke-width:1.1;opacity:.72;animation:brandScan 2.2s ease-in-out infinite}
    .ui-icon.blue{color:var(--blue);border-color:rgba(255,255,255,0.10);background:rgba(255,255,255,0.10)}
    .ui-icon.green{color:var(--green);border-color:rgba(34,197,94,.42);background:rgba(34,197,94,.10)}
    .ui-icon.red{color:var(--red);border-color:rgba(239,68,68,.42);background:rgba(239,68,68,.10)}
    .ui-icon.amber{color:var(--amber);border-color:rgba(245,182,66,.42);background:rgba(245,182,66,.10)}
    ...
    .intel-mini.green{border-color:rgba(34,197,94,.48)} .intel-mini.red{border-color:rgba(239,68,68,.48)} .intel-mini.amber{border-color:rgba(245,182,66,.48)} .intel-mini.blue{border-color:rgba(255,255,255,0.10)}
    ...
    .pill.green{border-color:rgba(34,197,94,.55);color:var(--green)} .pill.red{border-color:rgba(239,68,68,.55);color:var(--red)} .pill.amber{border-color:rgba(245,182,66,.55);color:var(--amber)} .pill.blue{border-color:rgba(255,255,255,0.10);color:var(--blue)}
    ...
    .panel-copy.primary{color:var(--text);font-size:.92rem}
    ...
    .decision-row.green{border-color:rgba(34,197,94,.34)} .decision-row.green .decision-row-value{color:var(--green)}
    .decision-row.red{border-color:rgba(239,68,68,.34)} .decision-row.red .decision-row-value{color:var(--red)}
    .decision-row.amber{border-color:rgba(245,182,66,.34)} .decision-row.amber .decision-row-value{color:var(--amber)}
    .decision-row.blue{border-color:rgba(255,255,255,0.10)} .decision-row.blue .decision-row-value{color:var(--blue)}
    .data-notice{border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:rgba(255,255,255,0.10);padding:11px 13px;color:#94A3B8;font-size:.9rem;margin:10px 0}
    .data-notice.warn{border-color:rgba(245,182,66,.34);background:rgba(245,182,66,.08);color:#F5B642}
    ...
    .structure-note{display:inline-flex;align-items:center;gap:8px;margin-top:14px;border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:rgba(255,255,255,0.10);padding:7px 10px;color:var(--blue);font-size:.82rem;font-weight:650}
    .structure-tile{border:1px solid var(--border);border-radius:8px;background:var(--surface);padding:12px;min-height:122px}
    .structure-tile.closest{border-color:var(--blue);box-shadow:none}
    ...
    .replay-shell{border:1px solid var(--border2);border-radius:8px;background:var(--surface);padding:14px;margin:10px 0 14px}
    ...
    .learning-hero{border:1px solid var(--border2);border-radius:8px;background:var(--surface);padding:16px}
    ...
    .briefing-shell{border:1px solid var(--border2);border-radius:8px;background:var(--surface);padding:16px;margin-top:12px}
    ...
    .briefing-body{white-space:pre-wrap;color:var(--text);font-size:.95rem;line-height:1.55;margin-top:14px}
    ...
    .source-card.connected{border-color:rgba(34,197,94,.38)} .source-card.unavailable{border-color:rgba(245,182,66,.34)}
    ...
    .morning-hero{position:relative;overflow:hidden;border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:var(--surface);padding:18px;margin:12px 0 14px}
    .morning-hero:before{content:"";position:absolute;inset:0;background:var(--surface);transform:translateX(-100%);animation:brandScan 4.5s ease-in-out infinite;pointer-events:none}
    ...
    .morning-orb{display:grid;place-items:center;justify-self:end;width:188px;max-width:100%;aspect-ratio:1;border-radius:999px;background:conic-gradient(var(--ring-color) calc(var(--confidence)*1%),rgba(141,160,184,.14) 0);box-shadow:none}
    .morning-orb-core{display:grid;place-items:center;width:78%;height:78%;border:1px solid rgba(255,255,255,.12);border-radius:999px;background:var(--surface);text-align:center;padding:14px}
    ...
    .morning-line-card{border:1px solid var(--border);border-radius:8px;background:var(--surface);padding:12px;min-height:142px;position:relative;overflow:hidden}
    ...
    .morning-line-card.call{border-color:rgba(34,197,94,.32);--wash:rgba(34,197,94,.18)}
    .morning-line-card.put{border-color:rgba(239,68,68,.34);--wash:rgba(239,68,68,.18)}
    .morning-line-card.neutral{--wash:rgba(255,255,255,0.10)}
    ...
    .decision-summary{border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:var(--surface);padding:12px;margin:12px 0}
    ...
    .decision-summary-chip.green{border-color:rgba(34,197,94,.35)}.decision-summary-chip.red{border-color:rgba(239,68,68,.35)}.decision-summary-chip.amber{border-color:rgba(245,182,66,.38)}.decision-summary-chip.blue{border-color:rgba(255,255,255,0.10)}
    .decision-summary-label{font-size:.72rem;font-weight:700;color:var(--muted);line-height:1.25}
    .decision-summary-value{font-size:.98rem;font-weight:780;color:var(--text);line-height:1.2;margin-top:5px}
    .morning-card{border:1px solid var(--border);border-radius:8px;background:var(--surface);padding:14px;min-height:168px;display:flex;flex-direction:column}
    .morning-card.green{border-color:rgba(34,197,94,.34)}.morning-card.red{border-color:rgba(239,68,68,.34)}.morning-card.amber{border-color:rgba(245,182,66,.36)}.morning-card.blue{border-color:rgba(255,255,255,0.10)}
    ...
    .brief-plan-shell{border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:var(--surface);padding:15px;margin:12px 0}
    ...
    .scenario-card.call{border-color:rgba(34,197,94,.34)}.scenario-card.put{border-color:rgba(239,68,68,.34)}.scenario-card.selected{box-shadow:none}
    ...
    .scenario-list span{border:1px solid rgba(141,160,184,.14);border-radius:7px;background:rgba(255,255,255,.035);padding:6px 7px;color:var(--text);font-size:.75rem;line-height:1.28}
    .scenario-list .caution{border-color:rgba(245,182,66,.28);color:#F5B642}
    .flow-board{border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:var(--surface);padding:14px;margin:12px 0}
    ...
    .flow-read-main{border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:rgba(255,255,255,0.10);padding:12px}
    .flow-read-main.call{border-color:rgba(34,197,94,.34);background:rgba(34,197,94,.09)}
    .flow-read-main.put{border-color:rgba(239,68,68,.34);background:rgba(239,68,68,.09)}
    .flow-read-main.wait{border-color:rgba(245,182,66,.38);background:rgba(245,182,66,.09)}
    ...
    .flow-card.darkpool{border-color:rgba(180,124,255,.32)}.flow-card.bull{border-color:rgba(34,197,94,.32)}.flow-card.bear{border-color:rgba(239,68,68,.32)}
    ...
    .flow-means{font-size:.76rem;line-height:1.34;color:var(--text);border-left:2px solid rgba(255,255,255,0.10);padding-left:8px;margin-top:9px}
    ...
    .action-brief{border:1px solid rgba(34,197,94,.34);border-radius:8px;background:var(--surface);padding:15px;margin:12px 0}
    .action-brief.wait{border-color:rgba(245,182,66,.4);background:var(--surface)}
    .action-brief.put{border-color:rgba(239,68,68,.38);background:var(--surface)}
    ...
    .morning-action{border:1px solid rgba(245,182,66,.34);border-radius:8px;background:var(--surface);padding:14px;margin:12px 0}
    ...
    .ai-verify{border:1px solid rgba(34,197,94,.24);border-radius:8px;background:var(--surface);padding:13px;margin:12px 0}
    ...
    .ai-verify-card.good{border-color:rgba(34,197,94,.34)}.ai-verify-card.warn{border-color:rgba(245,182,66,.34)}.ai-verify-card.info{border-color:rgba(255,255,255,0.10)}
    ...
    .morning-narrative{border:1px solid var(--border2);border-radius:8px;background:var(--surface);padding:14px;margin-top:12px}
    ...
    .morning-narrative-body{white-space:pre-wrap;color:var(--text);font-size:.9rem;line-height:1.55}
    .evidence-shell{border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:var(--surface);padding:14px;margin:12px 0}
    ...
    .evidence-card.connected{border-color:rgba(34,197,94,.3)}.evidence-card.watch{border-color:rgba(245,182,66,.34)}.evidence-card.internal{border-color:rgba(255,255,255,0.10)}
    ...
    .evidence-step-num{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;border-radius:999px;background:rgba(255,255,255,0.10);color:var(--blue);font-family:var(--mono-font);font-weight:900;font-size:.72rem}
    ...
    .source-row.connected{border-color:rgba(34,197,94,.28)}.source-row.unavailable{border-color:rgba(245,182,66,.28)}.source-row.scout{border-color:rgba(255,255,255,0.10)}
    .upgrade-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin-top:10px}
    .upgrade-card{border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:rgba(255,255,255,0.10);padding:9px}
    ...
    .daily-guide{border:1px solid rgba(255,255,255,0.10);border-radius:8px;background:var(--surface);padding:16px;margin:12px 0 14px}
    ...
    .daily-guide-copy{font-size:.92rem;color:var(--text);line-height:1.48;margin-top:9px}
    ...
    .daily-guide-label{font-size:.72rem;font-weight:750;color:var(--muted);line-height:1.25}
    ...
    .daily-guide-card.green{border-color:rgba(34,197,94,.34)}.daily-guide-card.red{border-color:rgba(239,68,68,.34)}.daily-guide-card.amber{border-color:rgba(245,182,66,.38)}.daily-guide-card.blue{border-color:rgba(255,255,255,0.10)}
    .daily-guide-card-title{font-size:.78rem;font-weight:850;color:var(--muted);line-height:1.25}
    ...
    .daily-guide-row-copy{font-size:.86rem;color:var(--text);line-height:1.4}
    ...
    .zone-call{border-color:rgba(34,197,94,.55)} .zone-put{border-color:rgba(239,68,68,.55)} .zone-neutral{border-color:rgba(255,255,255,0.10)}
    .signal-badge{display:inline-block;padding:3px 10px;border-radius:999px;font-size:.75rem;border:1px solid var(--border);margin-bottom:8px}.signal-call{background:rgba(34,197,94,.14)} .signal-put{background:rgba(239,68,68,.14)}
    .distance-wrap{height:7px;border-radius:99px;background:var(--surface3)}.distance-fill{height:7px;border-radius:99px;background:var(--surface)}
    *,*::before,*::after{box-sizing:border-box}
    ...
    .terminal-panel:hover,.structure-tile:hover,.brief-card:hover,.morning-line-card:hover,.morning-card:hover,.flow-card:hover,.evidence-card:hover,.daily-guide-card:hover,.option-quote-cell:hover{border-color:rgba(255,255,255,0.10);background-color:rgba(255,255,255,.05);box-shadow:none}
    ...
    .status-strip b{font-size:.72rem;color:var(--muted);text-transform:none;letter-spacing:0}
    ...
    [data-testid="stAlert"]{border-radius:8px;border:1px solid rgba(245,182,66,.26);background:rgba(245,182,66,.08);overflow-wrap:anywhere}
    [data-testid="stAlert"] p{font-size:.86rem;line-height:1.4}
    [data-testid="stDataFrame"]{border:1px solid var(--border);border-radius:8px;overflow:hidden;background:rgba(13,19,29,.72)}
    [data-testid="stDataFrame"] *{font-family:var(--ui-font)}
    div[data-testid="stDownloadButton"] button,div[data-testid="stButton"] button{border-radius:8px;border:1px solid rgba(255,255,255,0.10);background:rgba(16,24,38,.86);font-weight:760}
    div[data-testid="stDownloadButton"] button:hover,div[data-testid="stButton"] button:hover{border-color:rgba(255,255,255,0.10);background:rgba(255,255,255,0.10)}
...
/* === DESIGN SYSTEM LOCK (must come last) === */
html, body, .stApp, [data-testid="stAppViewContainer"]{
  background:#0B1220 !important;
  color:#E8ECF4 !important;
  font-family:"Inter","Manrope","Aptos","Segoe UI",system-ui,sans-serif !important;
}
[data-testid="stHeader"]{background:#0B1220 !important;border-bottom:1px solid rgba(255,255,255,0.06) !important;box-shadow:none !important}
[data-testid="stSidebar"], [data-testid="stSidebar"] > div, section[data-testid="stSidebar"]{
  background:#0B1220 !important;
...
[data-testid="stSidebar"] [data-testid="stWidgetLabel"], [data-testid="stSidebar"] label{
  color:#6B7B96 !important;font-size:11px !important;font-weight:600 !important;
  text-transform:uppercase !important;letter-spacing:0.08em !important;
}
[data-testid="stMarkdownContainer"] h1, [data-testid="stMarkdownContainer"] h2, [data-testid="stMarkdownContainer"] h3{color:#E8ECF4 !important}
...
[data-testid="stMetricLabel"]{color:#6B7B96 !important;font-size:11px !important;font-weight:600 !important;text-transform:uppercase !important;letter-spacing:0.08em !important}
[data-testid="stMetricValue"]{color:#E8ECF4 !important;font-size:24px !important;font-weight:700 !important}
...
.stTextInput input, .stNumberInput input, .stDateInput input, [data-baseweb="select"] > div{
  background:#131C2E !important;color:#E8ECF4 !important;
...
[data-baseweb="tab-list"] button[role="tab"][aria-selected="true"]{
  background:rgba(245,182,66,0.15) !important;
...
/* Card pattern: any class containing hero/card/shell/plate/tile/mini gets
   the unified card aesthetic — flat surface, subtle border, no shadow. */
[class*="hero"]:not(.hero-price):not([class*="hero-label"]):not([class*="hero-sub"]):not([class*="hero-arrow"]),
...
[class*="briefing-shell"], [class*="briefing-body"],
...
.flow-board-card, .kpi-card, .chart-card, .empty-state{
  background:#131C2E !important;
...
/* Label pattern */
[class*="-label"], [class*="-kicker"], [class*="-title"],
.brand-tagline, .market-clock, .panel-label, .tile-label, .hero-label{
  color:#6B7B96 !important;
}
.brand-title, .morning-title, .section-title, .action-headline,
.briefing-title, .daily-guide-title, .ai-verify-title, .decision-summary-title{
  color:#E8ECF4 !important;
}

/* Value pattern */
.hero-price, .decision-main, [class*="-value"], [class*="-main"]{
  font-family:"Inter",sans-serif !important;
}

/* Semantic colors */
.bull, .green, .connected, [class*="bull-"], [class*="-bull"]{color:#22C55E !important}
.bear, .red, [class*="bear-"], [class*="-bear"]{color:#EF4444 !important}
.amber, .caution, .warn{color:#F5B642 !important}
.neutral, .muted{color:#A0AEC8 !important}

/* Drop ALL pseudo-element decorative orbs/scans/orbits */
.brand-orbit, .brand-scan, .brand-pulse{display:none !important}
[class*="-orb"]:before, [class*="-orb"]:after{display:none !important}

/* Numeric / monospace contexts */
.hero-price, [class*="-mono"], [data-testid="stDataFrame"] [role="grid"]{
  font-family:"JetBrains Mono","Roboto Mono",Consolas,monospace !important;
}
...
def render_signal_card(signal):
    if signal is None:
        render_data_notice("No confirmed rejection at primary structure.", tone="info")
        return
...
def render_warning_panel(message): render_data_notice(message, tone="warn")
...
def unusual_whales_card_data(options: OptionsIntelligence) -> tuple[str, str, list[str], str]:
    ...
    if isinstance(contract_liquidity, dict) and contract_liquidity.get("contract_count"):
        chips.append(f"{contract_liquidity.get('contract_count')} contracts")
    if isinstance(whale_targets, dict) and whale_targets.get("target_count"):
        chips.append(f"{whale_targets.get('target_count')} whale targets")
    ...
    return value, copy, chips[:5], tone
...
def unusual_whales_gex_card_data(options: OptionsIntelligence) -> tuple[str, str, list[str], str] | None:
    ...
    source = str(gex.get("source") or "spot")
    copy = "Dealer hedging context from spot GEX by strike." if source == "spot" else "Dealer hedging context from static GEX by strike."
    if levels:
        copy = "Largest GEX strike " + fmt_price(levels[0].get("strike"), 0) + "; use as magnet/volatility context."
    net_dex = _finite_float(gex.get("net_dex"))
    if not pd.isna(net_dex) and net_dex:
        copy += f" Dealer delta exposure reads {fmt_money_short(net_dex)}."
    chips = [f"{fmt_price(row.get('strike'), 0)} GEX {fmt_money_short(row.get('total_gex'))}" for row in levels[:3]]
    for row in (gex.get("dex_levels") or [])[:2]:
        if isinstance(row, dict) and _finite_float(row.get("total_dex"), 0.0):
            chips.append(f"{fmt_price(row.get('strike'), 0)} DEX {fmt_money_short(row.get('total_dex'))}")
...
def order_flow_plain_english(options: OptionsIntelligence) -> dict:
    ...
    if side == "CALL":
        ...
        action = "Call setups get extra support only after price confirms at a structure trigger."
        tone = "call"
    elif side == "PUT":
        label = "Flow supports puts"
        action = "Put setups get extra support only after price confirms at a structure trigger."
        tone = "put"
    elif side == "MIXED":
        label = "Flow is mixed"
...
def order_flow_board_cards(options: OptionsIntelligence) -> list[dict]:
    ...
    if isinstance(whale_targets, dict) and whale_targets.get("target_count"):
        levels = []
        for row in (whale_targets.get("top_targets") or [])[:5]:
            if not isinstance(row, dict):
                continue
            levels.append({
                "label": f"{display_state_label(row.get('type'))} {fmt_price(row.get('strike'), 0)}",
                "value": fmt_money_short(row.get("premium")),
            })
        side = str(whale_targets.get("side") or "").upper()
        cards.append({
            "title": "Whale Targets",
            "value": str(whale_targets.get("tone") or "Whale target scan active"),
            "copy": f"{whale_targets.get('target_count', 0)} high-premium OTM SPY/index contracts; calls {fmt_money_short(whale_targets.get('call_premium'))}, puts {fmt_money_short(whale_targets.get('put_premium'))}.",
            "means": "High-premium OTM contracts show where paid traders are concentrating. Useful near a structure trigger; ignore levels far away from SPY Prophet entries.",
            "levels": levels,
            "tone": "bull" if side == "CALL" else "bear" if side == "PUT" else "",
        })
    if isinstance(contract_liquidity, dict) and contract_liquidity.get("contract_count"):
        top_call = next((row for row in contract_liquidity.get("top_calls") or [] if isinstance(row, dict)), None)
        top_put = next((row for row in contract_liquidity.get("top_puts") or [] if isinstance(row, dict)), None)
        levels = [
            {"label": "Top call", "value": f"{fmt_price(top_call.get('strike'), 0)}" if top_call else "-"},
            {"label": "Top put", "value": f"{fmt_price(top_put.get('strike'), 0)}" if top_put else "-"},
        ]
        for row in (contract_liquidity.get("liquid_strikes") or [])[:3]:
            if isinstance(row, dict):
                levels.append({"label": fmt_price(row.get("strike"), 0), "value": fmt_float(row.get("liquidity_score"), 0)})
        call_liq = _finite_float(contract_liquidity.get("call_volume"), 0.0) + _finite_float(contract_liquidity.get("call_open_interest"), 0.0)
        put_liq = _finite_float(contract_liquidity.get("put_volume"), 0.0) + _finite_float(contract_liquidity.get("put_open_interest"), 0.0)
        tone = "bull" if call_liq > put_liq * 1.15 else "bear" if put_liq > call_liq * 1.15 else ""
        cards.append({
            "title": "Contract Liquidity",
            "value": f"{contract_liquidity.get('contract_count', 0)} near contracts",
            "copy": f"Call liquidity {fmt_float(call_liq, 0)}; put liquidity {fmt_float(put_liq, 0)}.",
            "means": "Liquidity identifies tradable nearby strikes. It improves contract selection, but it does not replace the structure trigger.",
            "levels": levels,
            "tone": tone,
        })
    gex = whales.get("gex") or {}
    if isinstance(gex, dict) and (gex.get("levels") or gex.get("dex_levels") or not pd.isna(_finite_float(gex.get("net_gex")))):
        levels = []
        for row in (gex.get("levels") or [])[:3]:
            if isinstance(row, dict):
                levels.append({"label": f"GEX {fmt_price(row.get('strike'), 0)}", "value": fmt_money_short(row.get("total_gex"))})
        for row in (gex.get("dex_levels") or [])[:3]:
            if isinstance(row, dict) and _finite_float(row.get("total_dex"), 0.0):
                levels.append({"label": f"DEX {fmt_price(row.get('strike'), 0)}", "value": fmt_money_short(row.get("total_dex"))})
        cards.append({
            "title": "Dealer Exposure",
            "value": str(gex.get("dealer_tone") or "Dealer exposure active"),
            "copy": f"GEX {fmt_money_short(gex.get('net_gex'))}; DEX {fmt_money_short(gex.get('net_dex'))}.",
            "means": "GEX marks magnet or volatility zones. DEX adds directional dealer-hedging pressure. These levels support or caution entries only when they sit near a trigger.",
            "levels": levels[:5],
            "tone": "bull" if _finite_float(gex.get("net_dex"), 0.0) > 0 else "bear" if _finite_float(gex.get("net_dex"), 0.0) < 0 else "",
        })
    darkpool = whales.get("darkpool") or {}
        ...
        <div class='flow-board'>
          <div class='flow-board-head'>
            <div>
              ...
              <div class='flow-board-copy'>Premium market context: OTM SPY alerts, recent tape, market tide, contract liquidity, GEX, and dark-pool levels.</div>
            </div>
...
def render_briefing_evidence_trail(bundle: MorningBriefingBundle, result: MorningBriefingResult) -> None:
    ...
    whale_coverage = premium_feed_coverage(whales)
    whale_detail = (
        f"{whale_coverage.get('label')}; {whale_flow.get('alert_count', 0)} same-day SPY OTM alerts; {whale_flow.get('flow_bias', 'flow active')}; net pressure {fmt_money_short(whale_flow.get('net_premium_pressure'))}."
        if whale_flow
        else f"{whale_coverage.get('label')}; recent tape {whale_recent.get('tone')}; premium tape {whale_premium.get('tone')}."
    ...
    steps = [
        ...
        ("4", "Merge", "The recommendation must tie external context back to the four SPY Foresight structure lines."),
    ]
    step_html = "".join(
        ...
        <div class='daily-guide'>
          <div class='daily-guide-head'>
            <div class='daily-guide-main'>
              <div class='daily-guide-kicker'>Session Guide</div>
          ...
          <div class='daily-guide-section'>
            <div class='daily-guide-section-title'>External Context</div>
...
def render_daily_brief_tab(bundle: MorningBriefingBundle) -> None:
    render_section_title("Daily Brief", "Actionable guide for the current SPY Foresight assessment")
    ...
    with cols[0]:
        render_data_notice("Daily Brief converts the current assessment into the level to monitor, the confirmation rule, outside pressure, and invalidation.", tone="info")
    with cols[1]:
...
def render_morning_briefing_tab(bundle: MorningBriefingBundle) -> None:
    render_section_title("SPY Foresight", "Actionable same-day plan for the current structure lines")
    ai_ready = bool(get_secret_or_env("OPENAI_API_KEY"))
    active_bundle = st.session_state.get("morning_briefing_bundle")
    if not isinstance(active_bundle, MorningBriefingBundle) or pd.Timestamp(active_bundle.generated_at).date() != pd.Timestamp(bundle.generated_at).date():
        active_bundle = bundle
...
def main() -> None:
    ...
    option_state = None
    if df.empty:
        render_data_notice("SPY market data is pending. Refresh data or verify the market-data provider status.", tone="warn")
    if not df.empty:
    ...
    if show_debug:
        ...
        if _ds_selected_page == "Signal Details":
            ...
            if decision_state and decision_state.signal_quality:
                ...
                if q.warnings:
                    render_data_notice(", ".join(q.warnings).replace("_", " ").title(), tone="warn")
                if q.strengths:
                    render_data_notice(", ".join(q.strengths).replace("_", " ").title(), tone="info")
            if signals:
                signal_rows = []
                for sg in signals[-5:]:
                    signal_rows.append({
